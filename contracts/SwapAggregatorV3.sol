...
/**
 * @title SwapAggregatorV3
 * @notice Allowlisted multi-DEX execution layer for BaseHub swaps on Base.
 * @dev Frontend quotes routes off-chain. This contract executes one selected route,
 *      collects output-token fees, and sends the net amount to the user.
 */
contract SwapAggregatorV3 is Ownable, ReentrancyGuard {
    ...
    function setRouterSelector(address router, bytes4 selector, bool allowed) external onlyOwner {
    ...
    function executeSwap(
        ...
        bytes4 selector = _selector(routerCalldata);
        if (!allowedSelectors[router][selector]) revert InvalidSelector();
    ...
    function _collectInput(SwapContext memory context) internal {
        if (context.inputIsETH) {
            ...
            IWETH9(WETH).deposit{value: context.amountIn}();
        } else {
