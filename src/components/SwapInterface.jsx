...
import { DEFAULT_TOKENS, POPULAR_TOKENS, FEE_TIERS, searchTokens, getAllTokens, saveCustomToken, removeCustomToken, getTokenByAddress, BASE_CHAIN_ID } from '../config/tokens';
...
export default function SwapInterface() {
  ...
  const fetchCustomToken = async (contractAddress) => {
  ...
  const parseScientificNotation = (value, decimals = 18) => {
  ...
  const mobileOverrides = isMobile ? {
    header: { padding: '8px 12px', flexWrap: 'wrap' },
      ...
      <div style={getStyle(styles.mainContent, mobileOverrides.mainContent)}>
        ...
        {showStatistics && (
          <>
            ...
            />
            <div style={getStyle({ ...styles.statisticsModal, position: 'relative' }, mobileOverrides.statisticsModal)}>
              <button 
                onClick={() => setShowStatistics(false)} 
                style={{
                  position: 'absolute',
