...
export const useDeployERC1155 = () => {
  ...
  const deployERC1155 = async (name, symbol, uri) => {
    ...
    try {
      ...
      const initCode = ERC1155_BYTECODE + constructorData.slice(2)
      ...
      if (deployerAddress) {
        ...
        console.log('✅ ERC1155 deployed via deployer', contractAddress || '(check tx)')
      } else {
        const feeWallet = '0x7d2Ceb7a0e0C39A3d0f7B5b491659fDE4bb7BCFe'
        console.log('💰 Sending fee then deploying ERC1155...')
        if (!walletClient) throw new Error('Wallet not available. Please connect your wallet.')
